...
Sub-agents report up to one boss. Teammates argue with each other in the hallway. This chapter teaches you what changes when your AI workers start talking to each other — and shows you the small set of high-stakes problems where running a squad of agents categorically beats running a brilliant solo. By the end you will know the difference between the sub-agent and agent-team architectures, when each one wins, and how the `/goal` command lets you set a finish line and walk away.
...
The second half of the chapter introduces the `/goal` command — a completion condition the model checks after every turn using a separate evaluator. You define what "done" looks like, the team works without you, and the work stops the moment the evaluator confirms the condition is met. Paired with a team, `/goal` is the closest thing in current tooling to handing a deadline to a real team and walking away. The Lumenax Health case study returns to Priya and asks whether the lateral team architecture was really the right call, and what role `/goal` actually played in making the four-hour deadline survivable.
...
- **A VP of Marketing preparing a quarterly campaign review** would spin up a three-teammate team — one on paid performance across platforms, one on owned-channel engagement, one on competitive activity — set a `/goal` for "summary deck complete with five recommended Q-next bets," and walk away. By the time she returns, the cross-channel patterns have already surfaced because the teammates were talking to each other while they worked.
...
- The `/goal` command — this concept appears directly in the discussion prompt about Priya's decision.

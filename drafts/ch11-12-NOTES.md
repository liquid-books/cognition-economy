...
**Exercise (Move 1 + feature assumptions + submission mismatches).** Three tracks collapsed to one chat-surface exercise + three-surface-types admonition + **cognitioneconomy.net/ch12-companion** pointer. Track A's produce-judge-encode-recall loop kept as steps 1–5. Track B's session review folded in as step 6 using the audit's exact reframe: "Gather notes or transcripts from your last five significant AI sessions and paste them in…" — removes the silent assumption that the tool can "review the last five conversations we had this week" or enumerate "artifacts over the last seven days." Track B's Friday-calendar step kept as step 7 (evergreen).
...
1. **Companion pages to author:** cognitioneconomy.net/ch11-companion needs the Track B (Claude Code brief-drafting + stress-test) and Track C (Agent Manager) exercise steps, plus the current no-code platform list and the SDK doc link. cognitioneconomy.net/ch12-companion needs the Track B (context-file/CLAUDE.md) and Track C (project-rules) exercise variants — and when rebuilt, fix the submission-artifact naming so the steps create what the submission asks for.
...
- Audit said "Leave alone: all three exercise tracks"; the collapse was an unforced loss (flagged by the strategy doc). **Tracks B and C restored as close to the originals as possible**, applying only book-wide de-branding:
  - Three-track admonition restored using the ch09/ch10 standard "One Exercise, Three Surfaces" text (track letters + surface types + cognitioneconomy.net/ch11-companion).
...
- Ch 12 fell under the strategy doc's "keep collapsed" list, but Dr. Lee opted to restore; done in **DIAL style** (decision-named steps) rather than near-verbatim, since the originals carried two submission bugs (audit Bug 9):
  ...
  - **Verification lab added** (strategy doc 3.2 gap list — "ch12 light"): two active checks — does the lesson persist without your memory note (which memory layer do you control), and can you open your rules artifact as a file (if not, you don't own it) — closing with the cognitioneconomy.net/ch12-companion pointer.
...
- This **partially supersedes Flag 1** in the notes above: the companion pages now exist as content files; they still need to be published to cognitioneconomy.net and get QR codes in the print layout.

...
- **Move 1 (companion pages):** Tracks B and C deleted; three-track admonition replaced with one paragraph naming the three surface types + pointer to cognitioneconomy.net/ch03-companion. Track A kept in print (paste-a-prompt, evergreen) with its click-path softened to intent + companion pointer. All UI paths ("Customize → Connectors → +", "search 'Firecrawl MCP Claude Desktop'", "Project Settings → API"), doc-search instructions, and "five minutes/two minutes" timings moved to companion-site pointers. Config snippets now come "from the companion site."

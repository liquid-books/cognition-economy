...
- E) The user takes over and stops using the team; the fix is a goal condition
...
The chapter notes that your tool may let you either cycle through teammates one at a time in a single window or watch each teammate in its own pane. According to the chapter, how much does this choice matter?
...
- B) A goal condition can only be used with sub-agents, not with teams
...
- D) A goal condition forces every teammate to work in lockstep, which slows the team down

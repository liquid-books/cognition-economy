...
There is a third kind of trigger, and it completes the set. Hooks fire on events. Scheduled tasks fire on the clock. **Remote delegation** fires on *you* — from wherever you happen to be. The idea is a persistent channel between your phone and the agent on your desk: you send a task from the back of a cab or the hallway outside a meeting, the agent at your workstation picks it up, does the work with access to your actual files and tools, and the result is waiting when you get back — one continuous conversation, whether you are at the keyboard or a hundred miles from it. Anthropic's current name for this is Dispatch; the concept is what matters. With it, the automation stack covers all three triggers a professional actually has: things that happen (events), things that recur (schedules), and things that occur to you at the worst possible moment, nowhere near your desk (you). That last category is larger than most people think.

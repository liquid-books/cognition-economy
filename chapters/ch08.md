...
Open Antigravity 2.0 IDE, switch to the Editor surface, and open the integrated terminal with Ctrl+`. Launch Claude Code with `claude`. Plugins in Claude Code arrive through MCP — the Model Context Protocol — and they install with a single command.

1. **Web search via MCP.** In the Claude Code prompt, type `/mcp` to open the MCP plugin browser. Pick a web search plugin and install it. Confirm it loaded by asking a current-events question: *"What are the three biggest announcements in [your industry] in the last seven days?"* If Claude Code returns live results with citations, the plugin is working.

2. **Your most-used business tool.** Use `/mcp` again to install the MCP server for your CRM, project tracker, calendar, or notes app — whichever you reference most. Ask it for live data: *"What are the three most important things I need to do today based on my [tool name] data?"* The answer should come from the live system, not from memory.

3. **Verify in the file browser.** Any Artifacts the plugin produces — saved reports, exported tables, generated documents — appear in the Antigravity IDE file browser. Open one. Confirm it is real data, not a hallucination.

4. **Build one.** Have Claude Code scaffold a small MCP server for a narrow capability you wish you had — a custom search across your team's wiki, a wrapper around a vendor API. Save the spec inside the project so it is versionable and shareable.

### Track C — Gemini + Antigravity 2.0 IDE

Two Google products, two integration surfaces — one for conversation, one for agent tasks.

1. **Gemini Extensions.** Open Gemini at gemini.google.com. Go to Settings → Extensions and enable one — Google Workspace, YouTube, or Maps. Test with a question that requires live data: *"Summarize the last five emails from [client] and tell me what I should respond to first."* If Gemini answers from your actual inbox rather than guessing, the extension is alive.

2. **Antigravity Agent Manager tools.** Switch to Antigravity 2.0 IDE. Press CMD+E (Mac) or CTRL+E (Windows) to open the Agent Manager. Open a Project (or create one). Add a tool extension to the Project — search, browser actuation, or a Workspace connector. Run an Agent task that requires that tool: *"Pull this week's competitor announcements from the news and produce a one-page Artifact."* Open the resulting Artifact. The data should be current, sourced, and reviewable.

3. **Build one yourself.** Identify a narrow capability you wish you had. In Gemini, use the Gem framework or Apps Script to draft a small extension. In Antigravity, add a custom tool to your Project. Test it on real work.

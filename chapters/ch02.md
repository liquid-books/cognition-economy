...
Second, the desktop app is where the assistant stops being just a chat window and becomes an **agentic workspace** — a workshop bench rather than a search box. Anthropic's name for this workspace is Cowork; whatever it is called in your version, learn its shape, because the shape is the point. You connect it to a folder of real files, and it can read and edit them. It keeps saved procedures — skills — that it applies without being re-taught. It can browse the web in a sandbox, run tasks on a schedule, pick up work you send it from your phone while you are away from the desk, and organize work into projects. And when it produces a finished deliverable, the deliverable waits at an approval gate until you sign off. This is the *configured professional environment* of this chapter made literal: not a tool you visit, but a workspace where your context is already loaded and the work is already laid out. You will not use most of these capabilities this week. You will use all of them by the end of this book.

One more surface is worth naming now, because you will recognize it the moment you see it: the **artifact studio** — a surface where the deliverable is not text but a polished visual artifact. A design. A prototype. A slide deck. A one-pager you could hand to a client without apology. The conversation is still how you work — you describe, the AI proposes, you refine — but what comes out the other end is something you *show*, not something you read. Anthropic's current entry here is Claude Design; whatever your vendor calls its version, the shape is the same: the chat window stops being where finished work is described and becomes where finished work is *made*. Treat it like every other surface in this chapter — the product name is the least durable thing about it.

**Installing your thinking partner:**

Download the desktop application and sign in or create an account. The current download link, plan names, and prices are on the setup sheet at cognitioneconomy.net/ch02-companion.
...
Creating a Gem takes three moves: open the Gem manager, write instructions, name it and save. The exact buttons move between releases; the current click-path is on the setup sheet at cognitioneconomy.net/ch02-companion. Once saved, the Gem is permanently available in your sidebar.
...
Go to aistudio.google.com and sign in. Create an API key — the setup sheet at cognitioneconomy.net/ch02-companion shows exactly where that button lives this season — then copy it and store it somewhere safe. A password manager is ideal. You will need this key in later chapters.
...
**Your thinking partner:** its standing-instructions setting. Vendors rename this regularly — *preferences*, *custom instructions*, *project brief*; the setup sheet at cognitioneconomy.net/ch02-companion has the current name and location. This is your primary system prompt. Long, detailed, and focused on how you think and what you need from a thinking partner.

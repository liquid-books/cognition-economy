...
If you want the power of a terminal-native coding agent paired with a full IDE (file tree, diff viewer, integrated terminal) in one window, set up Claude Code inside the Antigravity 2.0 IDE. This is the "developer-flavored" version of the workshop — same idea as Track A, but everything lives in a real editor instead of a chat window.

1. Download and install **Antigravity 2.0 IDE** from [antigravity.google/docs/ide-overview](https://antigravity.google/docs/ide-overview). It is a VS Code-based IDE, so the layout will feel familiar.

2. Open the IDE. You land on the **Editor surface** — file tree on the left, editor in the middle, terminal at the bottom. Ignore Agent Manager for now.

3. Open the integrated terminal. Use **Ctrl+`** (backtick) or **View → Terminal**.

4. In the terminal, install Claude Code by following the official quickstart at [code.claude.com/docs/en/quickstart](https://code.claude.com/docs/en/quickstart). On macOS/Linux it is one curl command. Sign in when prompted.

5. Create a workshop folder anywhere on your machine — e.g. `~/cognitive-workshop` — and `cd` into it from the terminal.

6. Run the **meta-prompting interview**. With Claude Code running, type: *"Interview me with up to 8 questions, one at a time, then write me a complete system prompt for my recurring work."* Answer honestly and specifically.

7. When Claude Code delivers the finished prompt, save it as **`CLAUDE.md`** in the workshop folder. Claude Code loads that file automatically every session — your standing brief, in version control, no settings panel required.

### Track C — Gemini + Antigravity 2.0 IDE

This track uses **two separate Google products together**: Gemini for AI conversation work (especially anything that touches Google Workspace), and Antigravity 2.0 IDE's Agent Manager as a no-code orchestration surface for longer-running agent tasks. No terminal required.

1. **Gemini setup.** Go to [gemini.google.com](https://gemini.google.com) and sign in with your Google account. Click **Gem Manager → New Gem**. To draft the Gem instructions, open Claude Desktop in another tab and ask: *"Write me a Gemini Gem system prompt for [describe the task]. My role is [role]. Inputs look like [example]. Outputs should be [format/length/tone]."* Paste Claude's output into the Gem, name it, save it. Test it with one real example.

2. **Antigravity setup.** Download and install Antigravity 2.0 IDE from [antigravity.google/docs/ide-overview](https://antigravity.google/docs/ide-overview) if you have not already. Open it.

3. Press **CMD+E** (Mac) or **CTRL+E** (Windows) to switch from the Editor surface to the **Agent Manager** — the orchestration view designed for managing agents and artifacts rather than writing code.

4. In Agent Manager, create a **Project** for your most important current piece of work — the deal you are closing, the launch you are running, the hire you are making. Write the project-level instructions in plain English: who you are, what this project is, what good output looks like.

5. Run your first Agent task in the project. Pick something real and small — *"Summarize the state of this project and list the three things blocking it"* is a good starter. Watch it work asynchronously and produce an Artifact you can review.

You now have two complementary Google surfaces: Gemini for fast Workspace-aware chat, Antigravity Agent Manager for longer, project-scoped agent work.

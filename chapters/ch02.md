...
Before you can do anything useful with AI, you need tools installed and configured. Not browser bookmarks you open when you feel experimental — actual applications with your preferences saved, ready to work the moment you open them.

This chapter covers three tools. They are completely independent of each other. They do not share data. They do not communicate. They are not a system. They are three separate products made by three different companies, and you will choose between them based on what you are trying to do — not use them together.

Do not let the fact that you are installing them in the same chapter confuse you. The only reason they appear together is because you need all three before the rest of this book makes sense.

Install each one. Configure each one. Understand what it is for. Then stop thinking about them as a group.
...
Most importantly for the rest of this book: AI Studio is where you get your API key. An API key is a credential that lets other tools and scripts talk to Google's AI models directly. Without one, certain workflows simply are not possible. Getting your key now costs nothing and takes two minutes.
...
Click **Get API key** in the left sidebar → **Create API key** → copy the key → store it in a password manager or secure note. Do not put it in a document you share. Do not email it to yourself. Treat it like a password.

...
Open Antigravity 2.0 IDE, switch to the Editor surface, and open the integrated terminal with Ctrl+`. Launch Claude Code by typing `claude`. You now have Claude Code's reasoning paired with the IDE's file browser and diff viewer.

1. **Prompt Engineering.** In the Claude Code prompt, type a bare request: *"help me prep for a client meeting."* Read the result. Then rewrite the prompt with the client's name, the objective, the outcome, and the constraints. Run it again. The delta between the two outputs is the discipline.

2. **System Prompting.** Create a file named `CLAUDE.md` at the root of your project — Claude Code loads it as the system prompt for every session. Add your role, industry, and how you prefer briefings formatted. Save. Re-run the same Floor 1 prompt and watch the standing context reshape the answer.

3. **Meta Prompting.** Run `/agents` and ask Claude Code to design a *Meeting Prep* specialist agent — describe the inputs (client name, objective) and the expected structured brief. Save the agent definition. Invoke it on three upcoming meetings.

4. **Context Engineering.** Use the Antigravity IDE file browser to drop the client's last email, the prior proposal, and your interaction notes into the project. Reference them by name in your Claude Code prompt. Compare to a bare-name session — the difference is context made visible in your file tree.

5. **Memory Engineering.** Save your one-page Working Brief as `CLAUDE.md` in project scope. Claude Code now loads it every session, no pasting required. Test continuity across three separate sessions over a week.

6. **Harness Engineering.** Ask Claude Code to design the meeting-prep harness — triggers, data sources, outputs, delivery channel — and have it write the workflow map as a markdown file inside the project. Open it from the IDE file browser. The harness is now a file you can read, version, and hand to your team.

### Track C — Gemini + Antigravity 2.0 IDE

Open two windows: Gemini at gemini.google.com for conversational work, and Antigravity 2.0 IDE for orchestration. You will use Gemini for the prompt-through-context layers and the Antigravity Agent Manager for the harness layer.

1. **Prompt Engineering.** In Gemini, type a raw prompt: *"help me prep for a client meeting."* Read it. Now rewrite with specifics — client, objective, outcome, constraints. The Gemini answer sharpens immediately. That gap is prompt engineering.

2. **System Prompting.** Create a Gemini Gem (Gems → New Gem) for *Meeting Prep*. Paste standing instructions into the Gem: your role, your industry, your briefing format. The Gem now carries that context as its system prompt layer. Re-run the Floor 1 prompt inside the Gem and compare.

3. **Meta Prompting.** Open a fresh Gemini conversation and meta-prompt: *"Design a Meeting Prep Gem for me. I will provide a client name and meeting objective; it should return a structured brief covering background, likely concerns, questions to ask, and target outcome. Write the full Gem instructions."* Paste the result into a new Gem.

4. **Context Engineering.** In Gemini, click the attachment icon and share a Google Doc with the client's last proposal, or paste a Gmail thread. Run your meeting prep prompt against the attached context. Compare to a no-attachment session. The lift is context engineering — and Gemini's native Workspace integration makes it nearly frictionless.

5. **Memory Engineering.** Turn on Gemini's memory in Settings, or — more reliable — pin a *Working Brief* Gem that contains your one-page brief as its instruction set. Every conversation that runs through that Gem inherits your standing context as persistent memory.

6. **Harness Engineering.** Switch to Antigravity 2.0 IDE. Press CMD+E (Mac) or CTRL+E (Windows) to open the Agent Manager. Create a new Project named *Meeting Prep*. Paste your harness specification into the Project instructions. Add a Meeting Prep Agent that fires when you submit a calendar event title, pulls Workspace context, and produces a brief Artifact. The Agent Manager is your harness made concrete — orchestration without code.

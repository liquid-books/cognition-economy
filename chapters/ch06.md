...
Open Antigravity 2.0 IDE, switch to the Editor surface, and open the integrated terminal with Ctrl+`. Launch Claude Code by typing `claude`. Pick the same real task you chose above.

1. **State the objective without the approach.** In the Claude Code prompt, describe the goal, the audience, the constraints — but do not prescribe steps. Then explicitly invoke Plan Mode: *"Before any execution, give me a plan: steps, assumptions, risks, and what you would need to know to do this well. Wait for my approval."*

2. **Read the plan in the terminal.** Treat it as a contract. Push back on at least one assumption. Ask it to revise the sequence. Re-read.

3. **Approve and execute.** Say *"plan approved, proceed."* As Claude Code runs, the artifacts — new files, edits, diffs — appear in the Antigravity IDE file browser in real time. Open the diff viewer to see exactly what changed line by line. This is the verification surface that turns a plan into a reviewable build.

The discipline is the same as Track A. The difference is that the IDE makes the *execution itself* legible — every artifact of the plan is something you can open, read, and reject.

### Track C — Gemini + Antigravity 2.0 IDE

Plan Mode in the Agent Manager is the cleanest demonstration of why orchestration surfaces matter.

1. **Open the Agent Manager.** From Antigravity 2.0 IDE, press CMD+E (Mac) or CTRL+E (Windows) to switch to the Agent Manager view.

2. **Start a new async task.** In the task description box, prepend `/plan` before your full objective. Example: *"/plan — Draft the Q4 board memo for our renewal decision. Audience: five-person board. Constraints: under two pages, include the dissenting view fairly."*

3. **Review the plan Artifact.** The Agent will produce a plan Artifact before executing. Open it. Read it as a skeptic — what is it assuming, what is it skipping, what could go wrong. Push back inline.

4. **Approve and watch execution.** When you accept the plan, the Agent executes asynchronously. Watch the execution Artifacts populate — draft Artifact, references Artifact, change summary. For contrast, open a second workspace and run the same task without `/plan`. The side-by-side is the lesson: the planned version is auditable; the unplanned one is a guess.

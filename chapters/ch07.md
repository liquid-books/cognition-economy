...
Open Antigravity 2.0 IDE, go to the Editor surface, and open the integrated terminal with Ctrl+`. Launch Claude Code with `claude`.

1. **Shape the brief with Claude Code.** Paste your inventory answers into the Claude Code prompt and ask it to compress them into a clean, scannable Working Brief under 400 words. Read what it produces. Edit anything that does not sound like you.

2. **Save it as CLAUDE.md in the project root.** This is the file Claude Code automatically loads as memory at the start of every session in that project. No pasting. No reminding. The brief becomes the gravity of the workspace.

3. **Test continuity.** Open a fresh Claude Code session in the same project and ask it about something you are actively working on. The framing should sound like a colleague who already knows you — because it is reading your brief silently as system context.

4. **Set the monthly maintenance reminder.** Same as Track A. Calendar event: *Update CLAUDE.md.* Five minutes a month. That is the entire burden.

### Track C — Gemini + Antigravity 2.0 IDE

Persistent memory across two Google products at once — one for conversations, one for agent tasks.

1. **Gemini memory layer.** Create a new Gem called *Working Brief.* Paste your full Working Brief into the Gem's instructions field. This Gem now *is* your memory for any Gemini conversation that runs through it. Pin it. Every chat you start inside that Gem inherits your standing context.

2. **Antigravity memory layer.** Switch to Antigravity 2.0 IDE. Press CMD+E (Mac) or CTRL+E (Windows) to open the Agent Manager. Create a Project — name it for your current quarter or domain. Paste the Working Brief into the Project instructions field. Every Agent task that runs inside that Project reads the brief before doing anything.

3. **Verify both layers.** Ask a question in your Gemini Gem about a real decision you are weighing. Then create an async task in the Antigravity Project asking for a draft on the same topic. Compare. Both responses should reflect your context — one as a conversational reply, one as an Artifact.

4. **Maintenance.** Calendar event, once a month, titled *Update Working Brief — Gemini Gem + Antigravity Project.* Edit both. Five minutes total.
